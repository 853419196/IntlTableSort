...
function dateTimeTableSort(theadCell,setToTitleString,locales,options)
{
    ...
    for(let tbodyIndexNumber=0;tbodyIndexNumber<tableHTMLElement.tBodies.length;tbodyIndexNumber++)
    {
        ...
        if(tableHTMLElement.tBodies[tbodyIndexNumber].dataset.sortable)
        {
            ...
            if(setToTitleString)
            {
                ...
                rowInfoArray.forEach(function(rowInfo)
                {
                    ...
                    {
                        if(!isNaN(Date.parse(rowInfo[0].cells[cellIndexNumber].innerText)))rowInfo[0].cells[cellIndexNumber].title=tbodyFormatFunction(new Date(rowInfo[0].cells[cellIndexNumber].innerText));
            ...
            rowInfoArray.sort(function(upRowInfo,downRowInfo)
            {
                ...
                {
                    const downDateTimeNumber=Date.parse(downRowInfo[0].cells[downRowCellIndexNumber].innerText),upDateTimeNumber=Date.parse(upRowInfo[0].cells[upRowCellIndexNumber].innerText),resultNumber=isNaN(downDateTimeNumber)&&isNaN(upDateTimeNumber)?tbodyComparatorFunction(upRowInfo[0].cells[upRowCellIndexNumber].innerText,downRowInfo[0].cells[downRowCellIndexNumber].innerText):tableNumberComparatorFunction(upDateTimeNumber,downDateTimeNumber);
                    if(!resultNumber)
                    {
        ...
        else if(setToTitleString)
        {
            ...
            for(let localArray=[],rowIndexNumber=0,zeroRowSpanSet=new Set();rowIndexNumber<tableHTMLElement.tBodies[tbodyIndexNumber].rows.length;rowIndexNumber++)
            {
                ...
                {
                    if(localNumber+tableHTMLElement.tBodies[tbodyIndexNumber].rows[rowIndexNumber].cells[cellIndexNumber].colSpan>theadCell.dataset.local&&!isNaN(Date.parse(tableHTMLElement.tBodies[tbodyIndexNumber].rows[rowIndexNumber].cells[cellIndexNumber].innerText)))tableHTMLElement.tBodies[tbodyIndexNumber].rows[rowIndexNumber].cells[cellIndexNumber].title=tbodyFormatFunction(new Date(tableHTMLElement.tBodies[tbodyIndexNumber].rows[rowIndexNumber].cells[cellIndexNumber].innerText));
    ...
    if(tableHTMLElement.tFoot&&setToTitleString)
    {
        ...
        for(let localArray=[],rowIndexNumber=0,zeroRowSpanSet=new Set();rowIndexNumber<tableHTMLElement.tFoot.rows.length;rowIndexNumber++)
        {
            ...
            {
                if(localNumber+tableHTMLElement.tFoot.rows[rowIndexNumber].cells[cellIndexNumber].colSpan>theadCell.dataset.local&&!isNaN(Date.parse(tableHTMLElement.tFoot.rows[rowIndexNumber].cells[cellIndexNumber].innerText)))tableHTMLElement.tFoot.rows[rowIndexNumber].cells[cellIndexNumber].title=tfootFormatFunction(new Date(tableHTMLElement.tFoot.rows[rowIndexNumber].cells[cellIndexNumber].innerText));
