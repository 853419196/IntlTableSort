...
function initTable(theadCell)
{
    ...
    {
        for(let localArray=[],rowIndexNumber=0;rowIndexNumber<tableHTMLElement.tHead.rows.length;rowIndexNumber++)
        {
            if(!localArray[rowIndexNumber])localArray[rowIndexNumber]=[];
            for(let cellIndexNumber=0,scanPointNumber=0;cellIndexNumber<tableHTMLElement.tHead.rows[rowIndexNumber].cells.length;cellIndexNumber++)
            {
                while(localArray[rowIndexNumber][scanPointNumber])scanPointNumber++;
                tableHTMLElement.tHead.rows[rowIndexNumber].cells[cellIndexNumber].dataset.local=scanPointNumber;
                for(let scanRowNumber=0;scanRowNumber<tableHTMLElement.tHead.rows[rowIndexNumber].cells[cellIndexNumber].rowSpan;scanRowNumber++)
                {
                    ...
                    {
                        localArray[rowIndexNumber+scanRowNumber][scanPointNumber+scanColumnNumber]=true;
                    }
                }
                scanPointNumber+=tableHTMLElement.tHead.rows[rowIndexNumber].cells[cellIndexNumber].colSpan;
            }
        }
        tbody:
